...
export const StadiumGrid: React.FC<StadiumGridProps> = ({
  ...
  const getZoneColorClass = (zoneId: string) => {
    ...
    if (isSelected) {
      return 'fill-green-600 stroke-green-800 opacity-90 cursor-pointer';
    }

    // Default zone coloring based on general crowdLevel
    switch (crowdLevel) {
      case 'Critical':
        return 'fill-red-100 hover:fill-red-200 stroke-zinc-400 cursor-pointer';
      case 'High':
        return 'fill-orange-100 hover:fill-orange-200 stroke-zinc-400 cursor-pointer';
      case 'Medium':
        return 'fill-yellow-100 hover:fill-yellow-200 stroke-zinc-400 cursor-pointer';
      case 'Low':
      default:
        return 'fill-emerald-55 hover:fill-emerald-100 stroke-zinc-400 cursor-pointer';
    }
  };
    ...
    <div id="stadium-layout-container" className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-zinc-200 shadow-sm h-full">
      <div className="flex items-center justify-between w-full mb-3">
        <div>
          ...
          <p className="text-xs text-zinc-500">Estadio Azteca (Mexico City) – CDMX Hub</p>
        </div>
      ...
      <div className="relative w-full max-w-[420px] aspect-square flex items-center justify-center">
        <svg viewBox="0 0 400 400" className="w-full h-full select-none" id="stadium-svg">
          ...
          {/* Gate Markers (A - H Around Perimeter) */}
          {/* TOP / NORTH GATES */}
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate A', 'gate')}>
          ...
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate B', 'gate')}>
          ...
          {/* EAST GATES */}
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate C', 'gate')}>
          ...
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate D', 'gate')}>
          ...
          {/* SOUTH GATES */}
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate E', 'gate')}>
          ...
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate F', 'gate')}>
          ...
          {/* WEST GATES */}
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate G', 'gate')}>
          ...
          <g className="cursor-pointer" onClick={() => onSelectSector('Gate H', 'gate')}>
          ...
          <rect x="155" y="170" width="90" height="60" rx="2" className="fill-green-600 stroke-green-700 stroke-2 shadow-inner" />
          
          {/* Soccer Pitch Field Lines */}
          <rect x="160" y="173" width="80" height="54" className="fill-none stroke-white/40 stroke-[1.5]" />
          
          {/* Halfway line */}
          <line x1="200" y1="173" x2="200" y2="227" className="stroke-white/40 stroke-[1.5]" />
          <circle cx="200" cy="200" r="10" className="fill-none stroke-white/40 stroke-[1.5]" />
          
          {/* Penalty boxes */}
          <rect x="160" y="185" width="12" height="30" className="fill-none stroke-white/30 stroke-[1]" />
          <rect x="228" y="185" width="12" height="30" className="fill-none stroke-white/30 stroke-[1]" />

          {/* Goal areas */}
          <rect x="157" y="193" width="3" height="14" className="fill-none stroke-white/40 stroke-[1]" />
          <rect x="240" y="193" width="3" height="14" className="fill-none stroke-white/40 stroke-[1]" />

          {/* Center spot */}
          <circle cx="200" cy="200" r="1.5" className="fill-white" />

          {/* Static design text overlays */}
          <text x="200" y="204" textAnchor="middle" className="fill-white/20 text-[6px] font-bold tracking-widest font-sans uppercase pointer-events-none">Azteca 2026</text>
        </svg>
      </div>

      {/* Grid Legend & Interactive Details */}
      <div className="w-full mt-2 border-t border-zinc-100 pt-2 grid grid-cols-4 gap-1 text-[10px] text-zinc-500 font-sans">
      ...
      <div className="w-full mt-3 px-3 py-2 bg-zinc-50 rounded-lg border border-zinc-100 text-xs">
        {selectedSector ? (
          ...
          <div className="text-center text-zinc-400 italic">
            💡 Click on any Stand or Gate above to load coordinates
          </div>
        )}
      </div>
    </div>
